...
try:
    dane = open("C:\\ZnakiMniejszoci\\highscore.txt", "r").read()
    ustawienia = open("C:\\ZnakiMniejszoci\\ustawienia.txt", "r").readlines()
except FileNotFoundError:
    os.chdir("C:\\")
    os.mkdir("ZnakiMniejszoci")
    f = open("C:\\ZnakiMniejszoci\\highscore.txt", "w")
    f.write("0")
    f = open("C:\\ZnakiMniejszoci\\ustawienia.txt", "w")
    f.write('wysokość mapy: 5 \nokres spawnu: 3\nilość znaków naraz: 2\nznak gracza: "O"\nznak pusty: "."')
    f.close()
    dane = open("C:\\ZnakiMniejszoci\\highscore.txt", "r").read()
    ustawienia = open("C:\\ZnakiMniejszoci\\ustawienia.txt", "r").readlines()
highscore = int(dane)
...
def shs(ile):
    with open("C:\\ZnakiMniejszoci\\highscore.txt", "w") as f:
...
def run():
    ...
    if wynik > highscore:
        shs(wynik)
    print("PRZEGRAŁEŚ! Wynik:", wynik, "rekord:", highscore, )
    sleep(0.5)
    print('r - zagraj jeszcze raz\nq - opuść grę')
    czekajnaklawisz()
run()    
